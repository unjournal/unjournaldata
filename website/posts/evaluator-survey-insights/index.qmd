...
# Parse years in field - look for patterns like "15 years", "10+", "More than 10"
years_data <- survey %>%
  filter(!is.na(how_long_have_you_been_in_this_field) &
         how_long_have_you_been_in_this_field != "") %>%
  mutate(
    text = how_long_have_you_been_in_this_field,
    # Extract number that comes before "years" or is standalone
    years_numeric = case_when(
      # Match "X years" or "X+"
      str_detect(text, "\\d+\\s*(years?|\\+)") ~
        as.numeric(str_extract(text, "\\d+(?=\\s*(years?|\\+))")),
      # Match standalone numbers at start
      str_detect(text, "^\\d+") ~
        as.numeric(str_extract(text, "^\\d+")),
      # Match "than X" (for "more than X")
      str_detect(text, "than\\s+\\d+") ~
        as.numeric(str_extract(text, "(?<=than\\s)\\d+")),
      TRUE ~ NA_real_
    )
  ) %>%
  filter(!is.na(years_numeric), years_numeric < 100) # Remove outliers

# Parse papers reviewed - simpler approach
reviews_data <- survey %>%
  ...
  mutate(
    text = how_many_proposals_papers_and_projects_have_you_evaluated_reviewed_for_journals_grants_or_other_peer_review,
    # Extract any number (first occurrence)
    reviews_numeric = as.numeric(str_extract(text, "\\d+"))
  ) %>%
  filter(!is.na(reviews_numeric), reviews_numeric >= 5) # Remove very low counts
...
process_ratings <- survey %>%
  ...
  mutate(
    # Redact potentially identifying feedback
    how_would_you_rate_this_template_and_process = str_replace(
      how_would_you_rate_this_template_and_process,
      "I feel it was designed for other Unjournal work and less suitable for.*",
      "I feel it was designed for other Unjournal work and less suitable for... [redacted to avoid identification]"
    )
  )
...
n_field_responses <- length(expertise)
```

**`r n_field_responses` evaluators** provided their field of expertise, representing diverse backgrounds including development economics, experimental economics, environmental economics, political science, AI safety, meta-analysis, and more.
...
knitr::kable(
  ...
  caption = paste0(n_field_responses, " evaluator fields")
)
```
:::
...
We're continuing to improve the evaluation interface based on this feedback, analyzing our quantitative ratings to identify which metrics are most useful or redundant (see our [other data analysis posts](https://unjournal.github.io/unjournaldata/)). We're also seeking funding and partnerships to transition to a new evaluation platform, given PubPub's sun-setting. Looking ahead, we aim to provide tools helping evaluators better calibrate their ratings and predictions.
