...
- **11 papers verified** in institutional repositories via OpenAlex (3 evaluated, 8 pipeline) — actual numbers likely higher but difficult to verify systematically
...
hosts <- data.frame(
  ...
  All_Detected = c(113, 11, 66, 58, 49, 40, 39, 18, 18, 14, 5, 5, 1),
  All_Est = c("113", "11+‡", "66", "58", "50-60", "40", "80-100†", "18", "18", "14", "6-8", "5", "1"),
...
- *‡Institutional repos (verified via OpenAlex):* We queried OpenAlex for papers with DOIs and found 3 evaluated papers and 8 pipeline papers in institutional repositories. Actual numbers are likely higher since OpenAlex doesn't fully index all repositories, and ~100 papers lack DOIs. Primary platforms: DSpace (9 papers), eScholarship (3), PURE (1), HAL (1).
...
**Verified institutional repo presence (via OpenAlex):** We queried OpenAlex for all papers with DOIs to identify which appear in institutional repositories. Of 108 papers with DOIs checked:

- **3 evaluated papers** and **8 pipeline papers** appear in institutional repositories
- **DSpace**: 9 papers total (most common platform)
- **eScholarship (UC)**: 3 papers
- **Other institutional repos**: 2 papers (Warwick WRAP, Edinburgh Research Explorer)
- **PURE**: 1 paper
- **HAL**: 1 paper
- **EPrints**: Not detected via OpenAlex (though 2 papers have EPrints URLs as primary host)

*Caveats:* OpenAlex doesn't index all institutional repositories, and ~100 papers in our database lack DOIs. Actual institutional repo presence is likely higher but difficult to verify systematically. The primary URL analysis shows only ~4 papers hosted directly on institutional repo platforms (2 EPrints, 1 DSpace, 1 eScholarship).

### Specific University Repositories

Based on primary URL analysis, only **~4 papers** have institutional repositories as their primary host. Via OpenAlex cross-referencing, we verified **11 papers** (3 evaluated, 8 pipeline) appear in institutional repositories — though actual numbers are likely higher since OpenAlex doesn't fully index all repositories.
