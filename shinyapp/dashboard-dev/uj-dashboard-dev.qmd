...
```{r}
plotlyOutput("plot_comparison", height = "550px")
```
...
list(
  icon = "journal-text",
  color = "primary",
  value = textOutput("stat_papers_count", inline = TRUE)
)
```
...
list(
  ...
  color = "info",
  value = textOutput("stat_evals_count", inline = TRUE)
)
```

```{r}
#| content: valuebox
#| title: "Avg Hours/Evaluation"
list(
  icon = "clock",
  color = "success",
  value = textOutput("stat_avg_hours", inline = TRUE)
)
```

```{r}
#| content: valuebox
#| title: "% Anonymous Evaluators"
list(
  icon = "person-fill-lock",
  color = "warning",
  value = textOutput("stat_anon_pct", inline = TRUE)
)
```
...
Percentage of evaluators who chose to remain anonymous vs. those who are publicly named.

```{r}
plotlyOutput("plot_anonymity", height = "280px")
```
...
Distribution of hours evaluators reported spending on each evaluation (self-reported data, n=33).

```{r}
plotlyOutput("plot_time_spent", height = "280px")
```
...
Cumulative number of evaluations published by date.

```{r}
plotlyOutput("plot_evals_timeline", height = "250px")
```
...
output$plot_comparison <- renderPlotly({
  if (length(input$comparison_papers) == 0) {
    ...
    p |> plotly::layout(
      xaxis = list(
        title = list(text = "Percentile rank", font = list(size = 12)),
        range = c(0, 105),
      ...
      xaxis2 = list(
        title = list(text = "Journal tier", font = list(size = 12)),
        overlaying = "x",
        side = "top",
        range = c(0, 105),
...
# Calculate anonymity: evaluators starting with "Evaluator " are anonymous
anonymity_data <- reactive({
  unique_evaluators <- ratings |>
    filter(!is.na(evaluator)) |>
    distinct(evaluator) |>
    mutate(
      is_anonymous = str_detect(evaluator, "^Evaluator \\d+$")
    )

  list(
    total = nrow(unique_evaluators),
    anonymous = sum(unique_evaluators$is_anonymous),
    named = sum(!unique_evaluators$is_anonymous),
    pct_anonymous = round(100 * sum(unique_evaluators$is_anonymous) / nrow(unique_evaluators), 1)
  )
})

# Value box outputs
output$stat_papers_count <- renderText({
  n_papers()
})

output$stat_evals_count <- renderText({
  n_evals()
})

output$stat_avg_hours <- renderText({
  hours_data <- eval_paper_level$hours_spent_manual_impute
  hours_data <- hours_data[!is.na(hours_data)]
  if (length(hours_data) > 0) {
    paste0(round(mean(hours_data), 1), " hrs")
  } else {
    "N/A"
  }
})

output$stat_anon_pct <- renderText({
  paste0(anonymity_data()$pct_anonymous, "%")
})

# Anonymity pie chart
output$plot_anonymity <- renderPlotly({
  anon <- anonymity_data()

  plot_data <- data.frame(
    category = c("Anonymous", "Named"),
    count = c(anon$anonymous, anon$named),
    color = c("#FFC107", "#17A2B8")
  )

  plot_ly(plot_data, labels = ~category, values = ~count, type = 'pie',
...
output$plot_time_spent <- renderPlotly({
  ...
  plot_ly(hours_data, x = ~hours, type = "histogram",
    ...
    layout(
      ...
      yaxis = list(title = "Number of evaluations", tickfont = list(size = 11)),
      bargap = 0.1,
