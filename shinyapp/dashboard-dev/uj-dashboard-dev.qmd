  ...
  if (length(input$comparison_papers) == 0) {
    ...
    plot_data <- evals() |>
      ...
      mutate(
        ...
        # Reorder: Overall at top, journal tiers at bottom
        ...
        # Group for line breaks: separate journal tiers from other criteria
        crit_grp = criteria %in% c("Predicted journal tier", "Deserved journal tier")
      ) |>
      filter(!is.na(criteria))

    gg <- ggplot(plot_data, aes(x = rating, y = criteria, color = research,
                                 group = interaction(research, crit_grp),
                                 text = research)) +
      geom_point(size = 4) +
      # White line to visually separate journal tier from other criteria
      geom_hline(yintercept = 2.5, linewidth = 6, color = "white") +
      geom_line(orientation = "y", alpha = 0.7, linewidth = 1) +
    ...
    plotly::ggplotly(gg, tooltip = "text") |>
      plotly::layout(
        ...
        legend = list(orientation = "v", y = 1.2, x = 0, xanchor = "left")
      )
  }
})
