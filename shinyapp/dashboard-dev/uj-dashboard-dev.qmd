...
```{r}
plotlyOutput("plot_comparison", height = "650px")
```
...
# Interactive comparison plot with dual x-axes
# Uses ggplot2 for reliable vertical layout, then adds secondary axis via plotly
output$plot_comparison <- renderPlotly({
  if (length(input$comparison_papers) == 0) {
    ...
    plot_data <- evals() |>
      ...
      mutate(
        ...
        rating_scaled = ifelse(criteria %in% c("merits_journal", "journal_predict"),
                        rating * 20, rating),
        criteria_label = criteria_titles[criteria],
        # Set factor levels so Overall is at top, Predicted at bottom
        criteria_label = factor(criteria_label, levels = rev(criteria_order))
      ) |>
      filter(!is.na(criteria_label))

    # Create ggplot with proper vertical layout
    gg <- ggplot(plot_data, aes(x = rating_scaled, y = criteria_label, color = research,
      ...
      geom_point(size = 4) +
      geom_line(aes(group = research), orientation = "y", alpha = 0.6, linewidth = 1.2) +
      ...
      theme(
        ...
        axis.title.x.bottom = element_text(size = 13, margin = margin(t = 8)),
        axis.title.x.top = element_text(size = 13, margin = margin(b = 8)),
        legend.position = "top",
    ...
    # Convert to plotly - note: sec_axis may not convert perfectly,
    # so we add the secondary axis manually via layout
    p <- plotly::ggplotly(gg, tooltip = "text")

    # Add secondary x-axis at top showing Journal tier scale
    p |> plotly::layout(
      ...
      margin = list(t = 50, l = 200, b = 50, r = 20),
      xaxis = list(
        title = list(text = "Percentile rank", font = list(size = 13)),
        side = "bottom"
      ),
      xaxis2 = list(
        title = list(text = "Journal tier", font = list(size = 13)),
        overlaying = "x",
        side = "top",
        range = c(0, 100),
