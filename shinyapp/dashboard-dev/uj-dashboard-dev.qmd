  ...
  if (length(input$comparison_papers) == 0) {
    ...
    for (i in seq_along(papers)) {
      ...
      p <- p |> plotly::add_trace(
        ...
        type = "scatter",
        mode = "markers",
        marker = list(color = colors[i], size = 12),
        name = papers[i],
    ...
    # Add invisible trace on bottom axis to force it to show
    p <- p |> plotly::add_trace(
      x = c(20, 100),
      y = c(criteria_order[1], criteria_order[1]),
      type = "scatter",
      mode = "markers",
    ...
    p |> plotly::layout(
      title = list(text = "Paper comparison across criteria", font = list(size = 18)),
      xaxis = list(
