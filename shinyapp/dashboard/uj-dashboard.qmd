...
If visualizations aren't loading, see our <a href="https://unjournal.github.io/unjournaldata/posts/dashboard-data-jan2026/" target="_blank">static data page</a> for tables and plots.
</div>
...
Overall assessments for each paper by research area. Points show individual paper ratings; crosses show area averages.

```{r}
plotOutput("plot_ratings_static")
```
...
Detailed evaluations of [different aspects of research quality](https://globalimpact.gitbook.io/the-unjournal-project-and-communication-space/policies-projects-evaluation-workflow/evaluation/guidelines-for-evaluators). Crosses show per-question averages.

```{r}
plotOutput("plot_criteria_static")
```
...
```{r}
plotOutput("plot_comparison_static")
```
...
# Static plot for Overall Ratings
output$plot_ratings_static <- renderPlot({
  plot_data <- evals() |>
...
output$plot_criteria_static <- renderPlot({
  ...
  if (nrow(plot_data2) == 0) {
    ggp2 <- ggplot() +
      annotate("text", x = 2.5, y = 0.5, label = "No journal tier data", size = 4) +
      theme_void()
  } else {
    means2 <- plot_data2 |>
      group_by(criteria_label) |>
    ...
    ggp2 <- ggplot(plot_data2, aes(y = criteria_label, x = middle_rating, color = criteria_label)) +
      geom_jitter(height = 0.2, width = 0, alpha = 0.6, size = 2) +
      geom_point(data = means2, aes(x = mean_rating, y = criteria_label),
                 shape = 4, size = 3, stroke = 2, color = "black", inherit.aes = FALSE) +
  ...
  cowplot::plot_grid(ggp1, ggp2, nrow = 2, rel_heights = c(6, 2.5), align = "v")
})
...
output$plot_comparison_static <- renderPlot({
    ...
    plot_data <- evals() |>
      ...
      mutate(
        rating_scaled = ifelse(criteria %in% c("merits_journal", "journal_predict"),
                        rating * 20, rating),
        criteria_label = criteria_titles[criteria],
        criteria_label = factor(criteria_label, levels = rev(criteria_titles))
      ) |>
      filter(!is.na(criteria_label))

    ggplot(plot_data, aes(x = rating_scaled, y = criteria_label, color = research)) +
      geom_point(size = 4) +
      geom_line(aes(group = research), orientation = "y", alpha = 0.5) +
      scale_x_continuous(limits = c(0, 100), breaks = seq(0, 100, 20)) +
      labs(
        title = "Paper comparison across criteria",
        x = "Rating (scaled to 0-100)",
        y = NULL,
        color = "Paper"
      ) +
      theme(
