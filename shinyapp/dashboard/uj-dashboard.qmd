...
/* Reduce vertical spacing in Paper comparison section */
.card-body {
...
Compare selected papers across all evaluation criteria. Top axis shows percentile rank (0-100), bottom axis shows journal tier (0-5).
...
# Interactive comparison plot (plotly)
output$plot_comparison <- renderPlotly({
  if (length(input$comparison_papers) == 0) {
    plotly::ggplotly(
      ggplot() +
        annotate("text", x = 0.5, y = 0.5,
                 label = "Select papers above to compare", size = 6) +
        theme_void()
    )
  } else {
    # Define criteria order (top to bottom in plot)
    criteria_order <- c(
      "Overall assessment",
    ...
    plot_data <- evals() |>
      ...
      mutate(
        ...
        rating_scaled = ifelse(criteria %in% c("merits_journal", "journal_predict"),
                        rating * 20, rating),
        criteria_label = criteria_titles[criteria],
        # Factor levels: first item appears at TOP of plot
        criteria_label = factor(criteria_label, levels = criteria_order)
      ) |>
      filter(!is.na(criteria_label)) |>
      # Sort by research and criteria order to ensure lines connect properly
      arrange(research, criteria_label)

    # Calculate median across ALL papers for reference
    all_medians <- res_ratings |>
      summarize(.by = criteria, rating = mean(middle_rating, na.rm = TRUE)) |>
      mutate(
        rating_scaled = ifelse(criteria %in% c("merits_journal", "journal_predict"),
                        rating * 20, rating),
        criteria_label = criteria_titles[criteria],
      ...
      filter(!is.na(criteria_label)) |>
      summarize(.by = criteria_label, median_all = median(rating_scaled, na.rm = TRUE))

    # Calculate median for selected research area (if not "All")
    if (input$research_area != "All") {
      area_medians <- res_ratings |>
        filter(main_cause_cat_abbrev == input$research_area) |>
        summarize(.by = criteria, rating = mean(middle_rating, na.rm = TRUE)) |>
        mutate(
          rating_scaled = ifelse(criteria %in% c("merits_journal", "journal_predict"),
                          rating * 20, rating),
        ...
        filter(!is.na(criteria_label)) |>
        summarize(.by = criteria_label, median_area = median(rating_scaled, na.rm = TRUE))

      all_medians <- left_join(all_medians, area_medians, by = "criteria_label")
    }

    gg <- ggplot(plot_data, aes(x = rating_scaled, y = criteria_label, color = research,
                                 text = paste0(research, "<br>Rating: ", round(rating_scaled, 1)))) +
      # Add median reference line for all papers (grey)
      geom_point(data = all_medians, aes(x = median_all, y = criteria_label,
                 text = paste0("All papers median: ", round(median_all, 1))),
                 shape = "|", size = 6, color = "grey40", inherit.aes = FALSE) +
      geom_line(aes(group = research), alpha = 0.5, linewidth = 1) +
      geom_point(size = 4) +
      scale_x_continuous(
        ...
        breaks = seq(0, 100, 20),
        position = "top",
        sec.axis = sec_axis(~ . / 20, name = "Journal tier", breaks = 0:5)
      ) +
      scale_y_discrete(limits = rev(criteria_order)) +
      labs(
        title = "Paper comparison across criteria",
        y = NULL,
        color = NULL
      ) +
      theme(
    ...
    # Add area median if research area selected
    if (input$research_area != "All" && "median_area" %in% names(all_medians)) {
      gg <- gg +
        geom_point(data = all_medians, aes(x = median_area, y = criteria_label,
                   text = paste0(input$research_area, " median: ", round(median_area, 1))),
                   shape = "|", size = 6, color = "steelblue", inherit.aes = FALSE)
    }
