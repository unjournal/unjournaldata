...
Select one or more papers to compare them directly.

```{r}
selectizeInput("comparison_papers", label = "Choose papers", 
...
output$plot_comparison <- renderGirafe({
  ggp <- evals() |> 
    ...
    filter(! is.na(criteria)) |> 
    ggplot(aes(x = rating, y = criteria, color = research, group = research)) + 
      ggiraph::geom_point_interactive(
        aes(tooltip = research)
      ) +
