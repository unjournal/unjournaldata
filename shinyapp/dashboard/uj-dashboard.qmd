...
Compare selected papers against each other and median values. Grey | marks show the median across all papers; blue | marks show the median for the selected research area.
...
output$plot_comparison <- renderPlotly({
  if (length(input$comparison_papers) == 0) {
    ...
    gg <- ggplot(plot_data, aes(x = rating_scaled, y = criteria_label, color = research,
      ...
      geom_point(size = 5) +
      geom_line(aes(group = research), orientation = "y", alpha = 0.5, linewidth = 1) +
      scale_x_continuous(limits = c(0, 100), breaks = seq(0, 100, 20)) +
      labs(
        title = "Paper comparison across criteria",
        subtitle = "Grey | = median (all papers)",
    ...
    if (input$research_area != "All" && "median_area" %in% names(all_medians)) {
      gg <- gg +
        ...
        labs(subtitle = paste0("Grey | = all papers median, Blue | = ", input$research_area, " median"))
    }
