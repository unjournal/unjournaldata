...
# paper_ratings %>% 
#   filter(rating_type == "real_world") %>% 
#   group_by(user_name, paper_id) %>% 
#   filter(sum(is.na(value))==0) %>% 
#   ungroup() -> temp

# DistributionWAggMOD(expert_judgements = temp, round_2_filter = FALSE, type = "DistribArMean", percent_toggle = T)
...
DistributionWAggMOD(expert_judgements = temp$data[[5]], round_2_filter = FALSE, type = "DistribArMean", percent_toggle = T)
