...
evals_pub <- evals %>% 
  ...
  dplyr::filter(stage_of_process == "published evaluations (on PubPub, by Unjournal)") %>%
    select(id, 
           crucial_research, 
           paper_abbrev, 
           evaluator_name, 
...
# JB NOTE: I had to remove some of the columns that get selected here.
# It looks like the names have been changed in Airtable but this data cleaning
# script has not been updated accordingly, so the new data was not getting processed.
# If anything is missing, feel free to add it back with the correct name.
all_papers_p <- all_pub_records %>% 
  dplyr::select(
    id,
    category,
    main_cause_category,
    secondary_cause_category,
    # cfdc_assessor,
    # avg_cfdc,
    cause_cat_1_text,
    cause_cat_2_text,
