...
# Dot plot
ggplot(df, aes(fill = group, y = value, x = category)) + 
...
evaluations_table <- evals_pub %>%
  select(crucial_rsx, eval_name, cat_1, source_main, Overall, overall, adv_knowledge, methods, logic_comms, journal_predict)


out = formattable(
  evaluations_table,
  list(
    overall = color_bar("lightblue"),
    area(col = 6:9) ~ function(x) percent(x / 100, digits = 0),
    area(col = 6:9) ~ color_tile("#DeF7E9", "#3E7DCC")
  )
)
out
...
```{r}
df = data.frame("Type" = c("test"),
               Sparkline = c(as.character(htmltools::as.tags(sparkline(c(90,70,80, 60), type = "bullet")))))
