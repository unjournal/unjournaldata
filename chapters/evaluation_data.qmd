...
pub_records <- air_get(base = base_id, table = "crucial_research")
...
while(length(pub_records) == 100) {
  # Get the ID of the last record in the list
  last_id <- pub_records[[nrow(pub_records)]]$id
  
  # Fetch the next 100 records, starting after the last ID
  pub_records <- air_get(base = base_id, table = "crucial_research", filter_by_formula = paste0("RECORD_ID() > '", last_id, "'"))
  
  # Append the records to the list
  all_pub_records <- c(all_pub_records, pub_records)
}
...
#update to be automated
papers_considered = 200
papers_rejected = 10
papers_evaluated = 10
papers_complete = 7
papers_in_progress = 3
papers_still_in_consideration = 180
...
ggplot(summary_df, aes(x = category1, y = count, fill = category2)) +
  geom_bar(stat = "identity", position = "fill") + 
  coord_flip() + # This makes the chart horizontal
  theme_minimal() +
  labs(x = "Category 1", y = "Count", fill = "Category 2", 
       title = "Stacked bar chart of counts based on 2 categories") 
