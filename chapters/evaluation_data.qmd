...
```{r}
all_pub_records$is_evaluated = all_pub_records$`stage of process/todo` %in% c("published","contacting/awaiting_authors_response_to_evaluation","awaiting_publication_ME_comments","awaiting_evaluations") 
...
ggplot(aes(x = fct_infreq(source_main), fill = is_evaluated)) + 
  ...
  labs(title = "Prioritized and evaluated research by source") +
  theme_bw() +
  theme(text = element_text(size = 15)) +
